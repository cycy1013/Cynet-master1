...
class spatioTemporal:
    ...
    def __init__(self,
                 ...
                 month=None,
                 day=None,
                 EVENT='open', #列名,类型过滤所针对的列名;'Arrest'
                 coord1='Latitude',#列名
                 coord2='Longitude',#列名
                 coord3=None,
                 init_date=None,
        ...
        if log_file is not None:#指定了原始犯罪记录文件,就读它
            ...
            if year is not None and month is not None and day is not None:
                ...
                df=df.dropna()  #added by cy for 停止交易的日期有的是空
            print(df.info())
            df.to_pickle(log_store) #犯罪记录被写入:'stock.p'
        ...
        self._columns = [EVENT, DATE] #['Primary Type','Latitude','Longitude','Date']
        ...
        self._trng = self._logdf['date'].loc[(self._logdf['date']>=self._INIT) & (self._logdf['date']<=self._END)].values
        #print("self._trng:\n",self._trng)
    ...
    def getTS(self,_types=None,tile=None,freq=None,poly_tile=False,
        ...
        if self._value_limits is None:
            ...
            df = self._logdf[self._columns].sort_values(by=self._DATE).dropna()
            #print("df:\n",df)
        ...
        if freq is None:
            ...
            for i in range(len(self._trng) - 1):#最后一天的数据丢失了
                ...
                #print("df:\n",df) #有的日期就没有记录
                sub1 = df[(df.index >= start) & (df.index < end)]
                sub2 = df[(df.index > start) & (df.index <= end)]
                if sub1.values[0][0]==0:
                    if sub2.values[0][0]>0:
                        ts.append(1)
                    else:
                        ts.append(0)
                elif (sub2.values[0][0]-sub1.values[0][0])/sub1.values[0][0] >0.01: #涨了百分之一
                    ts.append(1)
                else:
                    ts.append(0)
                #print("ts:\n",ts)
    ...
    def timeseries(self,LAT=None,LON=None,EPS=None,_types=None,CSVfile='TS.csv',
        ...
        if tiles is not None or tiles is None :#我们的例子走这
            if auto_adjust_time:
                ...
                _TS = self.getTS(tile=None,_types=_types,poly_tile=poly_tile,local_func=self.local_func) # tqdm(tiles)
                #print("_TS:\n",_TS.info()) #(1,951)
...
def readTS(TSfile,csvNAME='TS1',BEG=None,END=None):
    ...
    for index, row in dfts.iterrows():#iterrows()：遍历df,并且返回索引和带着表头的数据行(内容row 是一个series,行标签是各列名)
        if len(row.unique()) != 1:
            ...
            # #一般不会走到这
            print("found one with all dupes") #所有的值都一样，这样的数据对训练无意义
            print("the row:\n",row,"\n 无效行的 index:\n",index)
...
class uNetworkModels:#一个模型文件对应一个这个对象
    ...
    def select(self,var="gamma",n=None,
        ...
        #M.select(var='delay',inplace=True,low=Horizon)
            ...
            #this_dict 用于排序
            print("low high 限定之前n:{}".format(len(self._models),end='\t'))
            this_dict={}  #{1:[],2:[],3:[]}
            #当var为delay时，VARNAME为ALL时的Model,会有多个delay相同，原来的做法会把相同的delay取最后一个，遗漏数据(modi by cy)
            #this_dict={value[var]:key
                       #for (key,value) in self._models.items() }
            for (key,value) in self._models.items():
                if value[var] not in this_dict:
                    this_dict[value[var]]=[key]
                else:
                    this_dict[value[var]].append(key)
            if low is not None:
                this_dict={key:this_dict[key] for key in list(this_dict.keys()) if key >= low }
                #this_dict[key]大概长这样:_amount_close_DEL_2.mod
            if high is not None:
                this_dict={key:this_dict[key] for key in list(this_dict.keys()) if key <= high }
            #print("low:\n",low)

            # if n is None:#this_dict里的value是个list,直接这么做数量就少了。
            #     n=len(this_dict)
            # if n > len(this_dict):
            #     n=len(this_dict)

            #调试用
            # if (len(self._models)>=70 and len(self._models)<=98):
            #     print("self._models:\n",self._models.keys(),"len(self._models):\n",len(self._models))
            #     #result_str2 = json.dumps(this_dict, indent=2)
            #     print("this_dict:\n", this_dict,"len(this_dict):\n",len(this_dict))
            #     #sys.exit()

            #out就排过序了,只取前n个模型
            out={}
            for k in sorted(list(this_dict.keys()),reverse=reverse):
                for innerlen in range(len(this_dict[k])):
                    out[this_dict[k][innerlen]] = self._models[this_dict[k][innerlen]]
                    if n is not None:
                        n = n - 1
                        if n == 0:
                            break
            ...
            #       for k in sorted(list(this_dict.keys()),reverse=reverse)[0:n]}
            #print("out:\n",out)
            print("low:{} high:{} 限定之后n:{}".format(low, high, len(out)))
...
class simulateModel:#模型已经生成，用它进行预测
    ...
    def __init__(self, MODEL_PATH,#stored_model:2model_sel_23a7b088-925f-41b6-bc75-01a4a3acc26e.json（把当前处理的model用src=varname进行筛选,用delay进行筛选，用gamma进行排序数量筛选后的模型文件）
    ...
    #并生成预测统计数据
    def run(self, LOG_PATH=None,#如:0modeluse31models#BURGLARY-THEFT-MOTOR_VEHICLE_THEFT.log,要生成的预测log文件
            PARTITION=0.5,#默认值0.5(改成1.0了)
            DATA_TYPE='continuous',#默认值 'continuous'??
            FLEXWIDTH=1,#默认值 是 1,1天的误差认为是正确的预测(改成0了)
            FLEX_TAIL_LEN=100,#传入是 365=>5
        ...
        #生成的log文件：0modeluse31models#BURGLARY-THEFT-MOTOR_VEHICLE_THEFT.log是:对一个目标块、目标变量（块+犯罪类型)，多个源块(31个)的一个源变量(BURGLARY-THEFT-MOTOR_VEHICLE_THEFT)的多个delta时延的预测
        ...
        print("flexstr_arg:\n", flexstr_arg,'\n 注意-w:flexwidth -x flex_tail_len -t tpr -f fpr',)
        ...
        print("LOG_PATH:",LOG_PATH,"生成的预测统计数据 auc:",auc,"tpr:",tpr,"fpr:",fpr)
        return auc, tpr, fpr
...
def flexroc_only(arguments):#处理一个log预测文件（每个模型(目标块+犯罪类型)的每类源变量都有一个log预测文件）,把这个预测log文件扩充成预测csv文件
    ...
    if custom_threshold:
        ...
        print("logfile:",logfile,"tpr_threshold:",tpr_threshold,"fpr_threshold",fpr_threshold,"得到的threshold:",threshold)
    with open(logfile,'r') as file:
        content = file.readlines()
    lines = []
    for line in content:#line: ----> open 7 1 0.671379 0.328621
        #print("预测log文件内容line:\n",line)
        elements = line.split()
        #Getting location
        ...
        # lines.append([lat1,lat2,lon1,lon2,variable,day,actual_event,negative_event,positive_event])
        lines.append([variable,day,actual_event,negative_event,positive_event])

    df = pd.DataFrame(lines, columns=header)
    #0modeluse31models#BURGLARY-THEFT-MOTOR_VEHICLE_THEFT.log
    #0modeluse85models#ALL.log
...
def flexroc_only_parallel(glob_path,#payload2015_2017/models/*.log,每个模型(目标块+犯罪类型)的每类源变量都有一个log预测文件
                cores = 4,
                FLEXWIDTH=0,#一天的误差被认为是预测正确,由1改成0
    ...
    '''
    In the event that we do not need to rerun cynet不需要重新运行cynet, but only flexroc on some files
    we will use this pipeline. This pipeline assumes that line has already
    been run and thus the cynet logfiles 预测log文件已经生成already exist. This function will apply
    flexroc to all of the cynet files and produce csvs生成预测csv文件 from them. These csvs are
    pandas friendly.
    Input-
    ...
    FLEXROC_PATH = os.path.dirname(sys.modules['stock2015_2017.cynet'].__file__) + '/bin/flexroc'
    #print("FLEXROC_PATH:\n",FLEXROC_PATH)
    args = []
...
def get_flexroc_threshold(cynet_logfile,FLEXROC_PATH, tpr=None, fpr=None,
        FLEXWIDTH=0,#预测一天的误差被认为是正确的,由1改成0
        FLEX_TAIL_LEN=100,
        POSITIVE_CLASS_COLUMN=5,
        EVENTCOL=3):
    '''
...
def parallel_process(arguments):#一个model文件跑一个这个进程
    ...
    PARTITION = arguments[10] #默认值0.5(改成1.0了)
    DATA_TYPE = arguments[11] #默认值 'continuous' ??
    FLEXWIDTH = arguments[12] #默认值 是 1,1天的误差认为是正确的预测(改成0了)
    FLEX_TAIL_LEN = arguments[13] #传入是 365=>5
    ...
    RESULT = []
    header=['loc_id','varsrc','vartgt','num_models','auc','tpr','fpr','horizon'] #是生成的预测统计res文件的头
    JSON_READ_PATH = os.path.dirname(sys.modules['stock2015_2017.cynet'].__file__) + '/bin/json_read' #目录下没有这个文件，没用到
    ...
    for varname in VARNAME:#针对一个模型文件，分别处理它的每一类src variable
        ...
        print("才读进来的M.models长度是:", len(M.models))
        if M.models:
            ...
            print("根据源变量:{}筛选后长度为:{}".format(varname,len(M.models)))
            M.select(var='delay',inplace=True,low=Horizon)
        ...
        if M.models:
            ...
            if testing:
                ...
                #如:0modeluse31models#BURGLARY-THEFT-MOTOR_VEHICLE_THEFT.log 是预测log文件
            ...
            PARTITION = PARTITION,#默认值0.5(改成1.0了)
            DATA_TYPE = DATA_TYPE,#默认值 'continuous' ??
            FLEXWIDTH = FLEXWIDTH,#默认值 是 1 ,1天的误差认为是正确的预测(改成0了)
            FLEX_TAIL_LEN = FLEX_TAIL_LEN,#传入是 365=>5
            ...
            result=[FILE]+[varname,varnametgt,model_nums,auc,tpr,fpr,Horizon] #FILE:./models/0model
...
def run_pipeline(glob_path,#*model.json #处理所有的模型文件，汇总每个模型的预测
                 model_nums,horizon,
                ...
                PARTITION=1.0,#?? 0.5
                DATA_TYPE='continuous', #??
                FLEXWIDTH=0,#1?? ,1天的误差认为是正确的预测 (改成0了)
                FLEX_TAIL_LEN=100,#传进来是 365=>5
                POSITIVE_CLASS_COLUMN=5,#事件发生的概率,从1开始计数,是log文件列的列号
                EVENTCOL=3,#事件是否发生了,从1开始计数,是log文件列的列号
                tpr_threshold=0.85,
                fpr_threshold=0.15,
                gamma=False,
                distance=False,
                res_filename='stock2015_2017/res_all.csv',
                READLEN=None,
                DERIVATIVE=0,#??
                CAP_P = False,
                phase = False):
    ...
    if RUNLEN == -1:#把split目录下的一条记录读进来看看有几列(即代表了有几天)
        RUNLEN = getSplitLen(glob.glob(DATA_PATH + '*')[0])
    ...
    CYNET_PATH = os.path.dirname(sys.modules['stock2015_2017.cynet'].__file__) + '/bin/cynet'
    #print("CYNET_PATH:\n",CYNET_PATH)
    FLEXROC_PATH = os.path.dirname(sys.modules['stock2015_2017.cynet'].__file__) + '/bin/flexroc'
    ...
    for model in models_files:
        for num in model_nums:#[31]  或[85]
            ...
            FLEXWIDTH,#就是默认值1,1天的误差认为是正确的预测(改成0了)
            FLEX_TAIL_LEN,#传进来是  365=>5
...
def get_var(res_csv, coords,varname='auc',VARNAMES=None):
    ...
    df1.to_csv("stock2015_2017/"+varname+Type+'.csv',sep=" ",index=None)
    plt.savefig("stock2015_2017/"+varname+Type+'.pdf',dpi=600, bbox_inches='tight',transparent=False)
...
class xgModels:
    ...
    def __init__(self, TS_PATH,
        ...
        if self.RUN_LOCAL:#本地运行时是通过模块cynet去找的，不是通过config.yaml中设置的
            #Find the local copy of XgenESeSS binary
            self.XgenESeSS_PATH = os.path.dirname(sys.modules['stock2015_2017.cynet'].__file__) \
            + '/bin/XgenESeSS'
    ...
    def run(self, calls_name='program_calls.txt', workers = 4, mpi=False):
        ...
        if self.RUN_LOCAL:
            commands = []
            command_count = 0
            for INDEX in np.arange(INDICES):#-g 0.01 大概是指 gamma相关系数要大于0.01,不然就去掉,现改成0.001
                #主要是靠 -k n 来区分处理的是哪一个目标块（块+犯罪类型）
                xgstr= self.XgenESeSS_PATH +' -f ' + self.TS_PATH\
                 ...
                 + self.LOG_PATH + ' -u '+ str(self.DERIVATIVE) +' -m -g -0.0001 -G 10000 -v 0 -A 1 -q -w '\
...
#['Cynet-master\\cynet/bin/XgenESeSS -f ./triplet/CRIME-_2015-01-01_2017-12-31.csv -k "  :1 "  -B 0  -E 60 -n 2 -p 0.5 -S -N ./triplet/CRIME-_2015-01-01_2017-12-31.coords -l  ./payload2015_2017/models/1log.txt -u 0 -m -g 0.01 -G 10000 -v 0 -A 1 -q -w ./payload2015_2017/models/1', 2]]
            ...
            if mpi:#用mpi的并行运行 (默认是False),这里是单机的并行运算
...
def run_oneXG(command):#处理一个块
    ...
    '''
    print("XgenESeSS Command {} has started".format(command[1])) #command[1]:这是序号
    args = shlex.split(command[0]) #linux下是这么写的,不然会找不到文件
    print("针对每个目标块生成模型",command[0][-1],"command[0]:\n",command[0])
    #print("args:\n",args)
    ...
    subprocess.check_output(args, shell=False) #win下是用:command[0]  linux下用args
    #print("XgenESeSS Command {} has finished".format(command[1]))
